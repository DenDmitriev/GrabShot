...
class GrabModel: ObservableObject {
    ...
    @Published var isGrabing: Bool
    @Published var status: Status
    @Published var grabingID: Video.ID?
    ...
    func cacalculate(for value: Video.Value) {
        self.isLoading = true
    ...
    func delete(for id: Int?) {
        ...
        DispatchQueue.main.async {
            ...
            let shotsDeletedVideo = Double(self.session.videos.first{ $0.id == id }?.shots ?? 0)
...
extension GrabModel: GrabOperationDelegate {
    ...
    private func allShotsCount() -> Int {
        ...
        case true:
            return 1
        case false:
            self.session.videos.forEach{ count += $0.shots + 1 }
            return count
        }
    }
